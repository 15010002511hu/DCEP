...
public class GwReqAspect {
	...
	@Pointcut("@annotation(com.dcep.dips.wholesalepayment.aspect.GwReq)")
	...
	public Object execute(ProceedingJoinPoint jp) throws Throwable {
		...
		try {
			...
			if(args[0] instanceof EnvelopeDTO){
				reqObj = (EnvelopeDTO<GwDTO>) args[0];
				msgSn = reqObj.getSoapHeader().getMsgSN();
			}


			// dto的msgSn值传入日志上下文中
			CommonUtil.LogMDC(msgSn);
			log.info("Wholesale gw Start:[" + invocation + "],startTime:[" + startTime.toString() + "],param:" + reqObj);
            ...
            if (elapse >= Integer.valueOf(TIMEOUT)) {
                timeoutLogger.info("Wholesale gw end:[" + invocation + "],elapse:" + elapse + "ms,result[" + msgSn + "]");
            }

            log.info("Wholesale gw end:[" + invocation + "],elapse:" + elapse + "ms,result[" + msgSn + "]:" + result);
			// 调用监控日志接入
			logInfoDigest(className, reqObj, msgSn, elapse, WholesaleErrorEnum.BUSI_SUCCESS.getCode());

			return result;
		} catch (DcepException dcepException) {
			log.error("Wholesale error", dcepException);
	...
	private void logInfoDigest(String className, EnvelopeDTO<GwDTO> reqObj, String msgSn, long elapse, String code) {
		try {
			...
			log.error("Wholesale gw req error Digest log: ", e);
		}
	}
