...
import com.emop.wlt.model.emap.emap322.Emap32200101ReqDTO;
...
import com.emop.wlt.user.management.mapi.business.CancelLossReportProvider;
...
@Service
public class CancelLossReportProviderImpl implements CancelLossReportProvider {
    ...
    @MappFlowJoin(nodeCode = "MAPP317", groupNodeCode = "BNC317", verifySceneType = "VST37", flowVersion = "VST37_01")
    @AppResponse
    @Override
    public ResponseModel<Mapp31700101Resp> preCheck(RequestModel<Mapp31700101Req> request) {
        ...
        if (CollectionUtils.isEmpty(walletInfoDTOList)) {
            ExceptionCast.buzCast(BaseErrorEnum.B06636);
        }
        ...
        Mapp31700101Resp mapp31700101Resp = new Mapp31700101Resp();
        mapp31700101Resp.setWalletBasicInfo(WalletInfoAssembleFactory.assemble(walletBasicInfoDTO));
        mapp31700101Resp.setCustomerIdInfo(WalletInfoAssembleFactory.assemble(walletInfoDTO.getCustomerIdInfo()));
        FlowRespInfo flowRespInfo = new FlowRespInfo();
        mapp31700101Resp.setFlowRespInfo(flowRespInfo);
        ...
        return ResponseModel.<Mapp31700101Resp>builder().messageBody(mapp31700101Resp).build();
    }

    @MappFlowJoin(nodeCode = "MAPP318", groupNodeCode = "BNC318", verifySceneType = "VST37", flowVersion = "VST37_01")
    @AppResponse
    @Override
    public ResponseModel<Mapp31800101Resp> cancelLossReport(RequestModel<Mapp31800101Req> request) {
        ...
        Emap32200101ReqDTO emap32200101ReqDTO = new Emap32200101ReqDTO();
        emap32200101ReqDTO.setWalletId(walletId);
        emap32200101ReqDTO.setContextNo(contextNo);

        GwPojoMsgDTO gwPojoMsgDto = EmapDTOBuildHelper.createGwPojoMsgDTO(
                emap32200101ReqDTO, userId, InstNoUtils.getInstNm(walletId));
        userProvider.cancelLoss(userId);
        emapGatewayManager.send(gwPojoMsgDto);

        return ResponseModel.<Mapp31800101Resp>builder().messageBody(new Mapp31800101Resp()).build();
    }
