 ...
 *
 * @Author qinchaoyong
 * @date 2024-11-15 16:32:03
 */
@Data
@Gateway(
        msgTp = "dcep.052.001.01"
)
@JacksonXmlRootElement(
        localName = "CtrctCclRspn",
        namespace = "http://www.dcep.com/dcep/05200101/"
)
public class Dcep05200101DTO extends GwDTO implements DataEncryption {
    /**
     * 业务头组件
     */
    @JacksonXmlProperty(
    ...
    private List<String> fetchEncryptionFeatures() {
        ...
        if (this.ctrctCclRcrd != null) {
            if (this.ctrctCclRcrd.getCtrctPtyList() != null
            ...
            if (this.ctrctCclRcrd.getRmngFndList() != null
                    && this.ctrctCclRcrd.getRmngFndList().size() > 0) {
                for (RmngFnd rmngFnd : this.ctrctCclRcrd.getRmngFndList()) {
                    if (StringUtils.isNotBlank(rmngFnd.getCstmrNm())) {
                        data.add(rmngFnd.getCstmrNm());
                    }

                    if (StringUtils.isNotBlank(rmngFnd.getWltId())) {
                        data.add(rmngFnd.getWltId());
                    }
                }
            }
        }
