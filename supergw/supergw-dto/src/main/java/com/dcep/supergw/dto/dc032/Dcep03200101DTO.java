 ...
 *
 * @Author qinchaoyong
 * @date 2024-09-11 09:52:27
 */
@Data
@Gateway(
        msgTp = "dcep.032.001.01", isReturn = true
)
@JacksonXmlRootElement(
        localName = "PdctModfyAudtDtlsQryRspn",
        namespace = "http://www.dcep.com/dcep/03200101/"
)
@Check032Biz(groups = Priority.Lowest.class)
public class Dcep03200101DTO extends GwDTO implements DataEncryption {
    /**
     * 业务头组件
     */
    @JacksonXmlProperty(
     ...
     */
    @JacksonXmlProperty(
            localName = "PdctModfyAudtDtlInf"
    )
    @Valid
    private PdctModfyAudtDtlInf pdctModfyAudtDtlInf;
    ...
    private List<String> fetchEncryptionFeatures() {
        ...
        if (this.pdctModfyAudtDtlInf != null) {
            if (StringUtils.isNotBlank(pdctModfyAudtDtlInf.getCtctInf())) {
                data.add(pdctModfyAudtDtlInf.getCtctInf());
            }
        }
    ...
    private void encryptionFeaturesAssign(List<String> encryptionFeatures) {
        ...
        if (this.pdctModfyAudtDtlInf != null) {
            if (StringUtils.isNotBlank(pdctModfyAudtDtlInf.getCtctInf())) {
                pdctModfyAudtDtlInf.setCtctInf(encryptionFeatures.get(index++));
            }
        }
    }
}
