...
public class DcepHttpCallBack implements FutureCallback<HttpResponse> {
    ...
    public DcepHttpCallBack(FutureCallback<HttpResponse> callback, String name) throws BlockException {
        ...
        try {
            ...
            if (InfoCacheUtils.isHasAgentOrg(name)) {
                agentEntry = SphU.asyncEntry(InfoCacheUtils.fetchAgentInst(name), EntryType.OUT);
            }
            asyncEntry = SphU.asyncEntry(name, EntryType.OUT);
    ...
    public void failed(Exception ex) {
        try {
            //对异常进行计数
            Tracer.traceEntry(ex, agentEntry);
